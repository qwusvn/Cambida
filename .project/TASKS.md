...
- [x] 2026-09-23 — Tách riêng kênh Telegram xác thực bản quyền (License Key Group): cấu hình LICENSE_TELEGRAM_CHAT_ID trực tiếp trong 1.py trỏ về nhóm Telegram 'Key' (-1003849724906) với bot Hằng; hỗ trợ đọc đồng thời cả tin nhắn ghim lẫn mô tả nhóm; bảo toàn 100% telegram_chat_id (1547756222) trong config.json cho các cảnh báo và thông báo cá nhân; hỗ trợ xử lý lệnh từ cả 2 kênh (commit `f5ff0e3`).
- [x] 2026-09-23 — Tối ưu phát lại trên iOS Safari (iPhone) và đóng gói bản phát hành 2.1.3: bổ sung `-movflags +faststart` vào lệnh ghi hình camera RTSP định kỳ trong 1.py; nâng cấp spec, launcher, version_info và script đóng gói lên 2.1.3; build hoàn tất gói staging onedir `staging\2.1.3\CCTV_2.1.3.exe` (SHA-256: `B7378F4DF9452508071D6A5F12230B3626FEB4B042BD84C0AAC5D02786ECC7B4`) đầy đủ 7 sidecar HTML, ffmpeg.exe, launcher và DLL Dahua NetSDK, không chứa config/db nhạy cảm (commit `83bc5a8`, `a1d1705`).
- [x] 2026-09-23 — Tinh chỉnh Timeline đa nguồn (Local ưu tiên + NVR nhạt màu) và đóng gói phát hành sạch 2.1.3: ẩn nút chọn nguồn Local/NVR và nút hướng tua; timeline render song song Local (xanh chính `#1d72f2`, z-index 2) và NVR (xanh nhạt `#93c5fd`, z-index 1); thuật toán tua ưu tiên Local tuyệt đối, chỉ fallback sang NVR khi Local khuyết dữ liệu; backend `/list/cam` tự động kết hợp cả hai nguồn; làm sạch template `config.release.json` (`cameras: []`, `site.name: ""`, `site.tagline: ""`); build hoàn tất gói `staging\2.1.3` sạch 100% không kèm `config.json` (SHA-256 `CCTV_2.1.3.exe`: `1E94738F89898A709002C3C9CDF18817581BA99E592A7ABCA15BB125BAE5AB37`).
- [x] 2026-09-23 — Tối ưu phát lại iOS Safari, responsive desktop và đóng gói 2.1.4: trình phát xem trước màn hình Hoàn thành, lưu video iOS Photos qua Quick Look/Partial Content, vuốt timeline toàn thẻ, chặn nhảy trang overscroll, kẹp cận trên giờ cắt tránh lỗi HTTP 400. Đóng gói staging 2.1.4 sạch sẽ (commit `6c85c65`).
- [x] 2026-09-24 — Đóng gói phát hành 2.1.5 & Thiết lập quy tắc phiên bản theo commit: tích hợp module camera độc lập `camera_modules/` (Dahua/Imou, Hikvision, ONVIF, RTSP, Table Access, Policy), công tắc riêng tư bàn (Privacy Switch) trên `home.html` và backend `1.py` (Bật = Đỏ, Tắt = Xám), giao diện thêm camera đa hãng `admin.html`, giới hạn bàn qua Telegram pin `/N`, tự động chuyển hướng QR sang HTTPS và fallback HTTP LAN. Biên dịch và đóng gói hoàn tất vào `release/2.1.5` kèm file nén `release/2.1.5.zip` (SHA-256 EXE: `B4C575B47C987B2D1218E6231383884D2F4B4EF6547C5249D4221E37FD37C19F`). Kể từ bản này, mỗi commit tiếp theo sẽ tự động tịnh tiến phiên bản mới (2.1.6, 2.1.7...).
- [x] 2026-09-24 — Sửa lỗi cú pháp xem lại, tích hợp Cloudflared & Đóng gói 2.1.51: loại bỏ dấu `}` thừa trong `index.html` và template nhúng trong `1.py`, khôi phục date picker và timeline ruler (26/26 Node tests PASS). Tích hợp `cloudflared.exe` và `cloudflared_setup/` vào gói phát hành, đóng gói onedir `release/2.1.51` và file nén `release/2.1.51.zip` (SHA-256 EXE: `09BC1712A71C29E231691181EF7C06CB14C352E4DEE07F64344080143E70A562`). Khởi động lại server 8004.

- [x] 2026-09-24 — Chuyển trang tức thì khi cắt video, tiến trình 2 giai đoạn, pinch zoom timeline & Đóng gói 2.1.52: chuyển tức thì sang #doneScreen khi ấn Cắt; thanh tiến trình 2 giai đoạn (Giai đoạn 1 cắt server SSE, Giai đoạn 2 nạp ReadableStream vào RAM kèm đếm MB và phát tức thì); cử chỉ 2 ngón tay chụm/tách pinch-to-zoom timeline (1x - 96x). Cập nhật template nhúng 1.py; 28/28 Node tests PASS; đóng gói hoàn tất release/2.1.52 (SHA-256 EXE: EC56056A60CEB8AE08BCF567C0F5092BBBB3F9A3F99108D0BF34B1C3413D0527, ZIP: F9B25D203195A9C06BDF764492DA6DAEAC97F69257F33E96548B402876250920) kèm cloudflared.exe và cloudflared_setup/. Khởi động lại server 8004.
- [x] 2026-09-24 — Giao diện nút tích hợp tiến trình 50/50, zoom mặc định 3h/1h & Khắc phục lỗi vô tình mở video khi zoom timeline (2.1.53): loại bỏ popup/loading to trên #doneScreen, dồn tiến trình 50/50 trực tiếp vào nút Lưu và chia sẻ (.merge-button-fill), triệt tiêu tranh chấp capture/nổi bọt giữa timelineHit và timelineCard kèm cooldown 350ms ngăn chặn tự động seek/mở video khi pinch-to-zoom, đặt mặc định zoom xem lại 3h (zoom=16) và zoom cắt video 1h quanh kim (zoom=48); cập nhật template nhúng và SHA-256 trong 1.py; 28/28 Node tests PASS.
- [x] 2026-09-24 — Cố định mã QR bàn mang IP nội bộ LAN, phân loại iOS sang Cloudflare, giữ Android ở mạng LAN & Phát hành Auto Update (2.1.60): gỡ bỏ nhúng link Cloudflare vào ảnh QR bàn tại table_qr; tự động xác định IP LAN qua get_local_lan_ip; đảm bảo quét QR trên Android ở lại mạng LAN nội bộ và chỉ iOS mới redirect sang Cloudflare HTTPS; ưu tiên Chay_CCTV.cmd trong updater.cmd; biên dịch CCTV_2.1.60.exe, đóng gói zip cập nhật release/2.1.60.zip và phát hành GitHub Release v2.1.60 phục vụ tự động cập nhật.
## DOING
