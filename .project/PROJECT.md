...
- Codex là executor chính cho coding/core nhưng hiện chỉ được khởi chạy qua Codex CLI; mặc định dùng lane/model đã cấu hình cho Codex CLI.
- AGY dùng cho khảo sát/UI/prototype/test/module độc lập nhưng hiện chỉ được khởi chạy qua AGY CLI.
- Yato/process manager chịu trách nhiệm khởi chạy và giữ tiến trình CLI dài hạn, system/ADB/process/service/port/environment/script/recovery.
- Remote chỉ dùng khi cần GUI/visual; không thay thế AGY/Codex CLI nếu chưa có chỉ thị của người dùng.
- Không dùng direct AGY/Codex MCP hoặc agent_start để thực thi task; chỉ quay lại cơ chế đó khi người dùng yêu cầu rõ ràng.
- Mỗi task AGY/Codex phải giữ cả `task_id` và `process_id`, theo dõi log/status xuyên suốt tới kết quả terminal thực sự.
...
- AGY/Codex CLI-only, theo dõi cùng tiến trình tới kết quả thực được xác minh; checkpoint 15 phút, Telegram gộp theo parent; một final event khi terminal.
