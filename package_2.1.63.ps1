...
try {
    ...
    # Sidecars: Chỉ chứa HTML, launcher, updater, config template; KHÔNG chứa 1.py
    $Sidecars = @(
        "index.html",
        "admin.html",
        "admin_login.html",
        ...
        "RELEASE_VERSION.txt",
        "$Name.launcher.cmd",
        "CCTV_2.1.62.launcher.cmd",
        "CCTV_2.1.61.launcher.cmd",
        "CCTV_2.1.60.launcher.cmd",
        "CCTV_2.1.53.launcher.cmd"
    )
    ...
    Copy-Item -LiteralPath (Join-Path $Root "Chay_CCTV.cmd") -Destination (Join-Path $Stage "Chay_CCTV.cmd") -Force
    Copy-Item -LiteralPath (Join-Path $Root "RELEASE_VERSION.txt") -Destination (Join-Path $Stage "VERSION.txt") -Force
...
This package intentionally contains no operational config.json and no external 1.py.
Backend Python code is 100% bundled within $Name.exe and _internal.
...
- This is a staging onedir build with Cloudflared, complete Python runtime bundled into $Name.exe.
- No production config, database, logs, media, cache, or raw python source files are bundled.
...
"@ | Set-Content -LiteralPath (Join-Path $Stage "README_FIRST_RUN.txt") -Encoding UTF8
    ...
    Write-Host "[3/6] Enforcing Zero Config Pollution & No Source Leak..." -ForegroundColor Cyan
    $Forbidden = @("config.json","analytics.db","cctv_videos","logs","nvr_cache","1.py")
    foreach ($forbiddenName in $Forbidden) {
        if (Test-Path -LiteralPath (Join-Path $Stage $forbiddenName)) {
            throw "Forbidden runtime/production/source item found in staging root: $forbiddenName"
        }
    }
    $Required = @(
